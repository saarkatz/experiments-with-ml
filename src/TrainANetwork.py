...
def train_new_net(num_games, callback=(lambda x, y, z: print(x) if x % 100 == 0 else None)):
    ...
    for i in range(num_games):
        engine.init()
        result = engine.run()
        ai_agent = engine.player0
        turn_queue = engine.turn_queue
        if callback:
            callback(i, result, engine)
        winner = result[0] if result[2] == 1 else 1 - result[0]
        dataset = [(x[2].flatten(), x[3]) for x in turn_queue if x[0] % 2 == winner]
        for input_vec, output_vec in dataset:
            ai_agent.nn.learn(input_vec, output_vec, iterations=1)
...
if __name__ == '__main__':
    engine = train_new_net(20000)
    engine.player0.nn.save('final_net')